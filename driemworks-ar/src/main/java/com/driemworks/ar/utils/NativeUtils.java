...
public class NativeUtils {
    ...
    /**
     * This is actually quite slow, as it makes a couple of native calls for each of the NativeUtils.getXXX methods...
     */
    @Deprecated
    static double getVar(final BoundingBox box, final Mat sum, final Mat sqsum) {
        final int brs = NativeUtils.getInt(box.y + box.height, box.x + box.width, sum);
        final int bls = NativeUtils.getInt(box.y + box.height, box.x, sum);
        final int trs = NativeUtils.getInt(box.y, box.x + box.width, sum);
        final int tls = NativeUtils.getInt(box.y, box.x, sum);
        final double brsq = NativeUtils.getDouble(box.y + box.height, box.x + box.width, sqsum);
        final double blsq = NativeUtils.getDouble(box.y + box.height, box.x, sqsum);
        final double trsq = NativeUtils.getDouble(box.y, box.x + box.width, sqsum);
        final double tlsq = NativeUtils.getDouble(box.y, box.x, sqsum);
    ...
    static byte getByte(final int row, final int col, final Mat mat) {
        if (CvType.CV_8UC1 != mat.type())
            throw new IllegalArgumentException("Expected type is CV_8UC1, we found: " + CvType.typeToString(mat.type()));
    ...
    static int[] getIntArray(final Mat mat) {
        if (CvType.CV_32SC1 != mat.type())
            throw new IllegalArgumentException("Expected type is CV_32SC1, we found: " + CvType.typeToString(mat.type()));
    ...
    static float[] getFloatArray(final Mat mat) {
        if (CvType.CV_32FC1 != mat.type())
            throw new IllegalArgumentException("Expected type is CV_32FC1, we found: " + CvType.typeToString(mat.type()));
    ...
    static double[] getDoubleArray(final Mat mat) {
        if (CvType.CV_64F != mat.type())
            throw new IllegalArgumentException("Expected type is CV_64F, we found: " + CvType.typeToString(mat.type()));
    ...
    public static float getFloat(final int row, final int col, final Mat mat) {
        if (CvType.CV_32F != mat.type())
            throw new IllegalArgumentException("Expected type is CV_32F, we found: " + CvType.typeToString(mat.type()));
