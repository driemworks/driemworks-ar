...
public class BoundingBox extends Rect {
    ...
    Point[] points() {
        ...
        Log.i(NativeUtils.TAG, "Points in BB: " + this + " stepx=" + stepx + " stepy=" + stepy + " RES size=" + result.size());
    ...
    BoundingBox predict(final Point[] points1, final Point[] points2) {
        if (points1.length != points2.length)
            throw new IllegalArgumentException("The 2 arrays of points must be of the same lenght ! (" + points1.length + ", " + points2.length + ")");

        final int npoints = points1.length;
        Log.i(NativeUtils.TAG, "Tracked points: " + npoints);
        ...
        final float dx = NativeUtils.median(xoff);
        final float dy = NativeUtils.median(yoff);
        ...
        if (npoints > 1) {
            ...
            for (int i = 0; i < npoints; i++) {
                for (int j = i + 1; j < npoints; j++) {
                    d[idx++] = NativeUtils.norm(points2[i], points2[j]) / NativeUtils.norm(points1[i], points1[j]);
                }
            }
            s = NativeUtils.median(d);
        }
        ...
        Log.i(NativeUtils.TAG, "Current BB: " + this + ", Predicted BB: " + result);
