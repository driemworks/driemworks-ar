 ...
 * @author Tony
 */
public class MonocularVisualOdometryServiceExperiment {
    ...
    private final boolean isSubmatMode;

    /**
     * The default constructor
     */
    public MonocularVisualOdometryServiceExperiment(Resolution resolution, boolean isSubmatMode) {
        super();
            ...
            if (!currentPoints2f.empty() && currentPoints2f.checkVector(2) > 0) {
                ...
                try {
                    ...
                    essentialMat = Calib3d.findEssentialMat(currentPoints2f, previousPoints2f,
                            FOCAL, PRINCIPAL_POINT, Calib3d.RANSAC, 0.99, 1.00, mask);
...
//                    if (0 <= touchedRect.x && touchedRect.x <= currentFrame.cols()
//                           && 0 <= touchedRect.y && touchedRect.y <= currentFrame.rows()) {
//                        Mat sharpenedCurrentFrame = OpenCvUtils.sharpenImage(currentFrame);
//                        currentPoints = correctKeyPoints(
//                                featureService.featureDetection(sharpenedCurrentFrame.submat(touchedRect)));
//                    }
